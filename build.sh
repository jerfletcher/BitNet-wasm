...
# Emscripten compiler flags - Use llama-cpp-wasm settings but keep RTTI for --bind compatibility  
EMCC_FLAGS="-O3 -msimd128 -DNDEBUG -flto=full -s BUILD_AS_WORKER=0 -s WASM=1 -s MODULARIZE=1 -s EXPORT_ES6=0 -s EXPORTED_RUNTIME_METHODS=['ccall','cwrap','HEAPU8','HEAPU32','HEAPF32','HEAP8','HEAP32','lengthBytesUTF8','stringToUTF8','UTF8ToString'] -s EXPORTED_FUNCTIONS=['_malloc','_free','_bitnet_init','_bitnet_load_model_from_memory','_bitnet_run_inference_simple','_bitnet_is_model_loaded','_bitnet_get_vocab_size','_bitnet_get_embedding_dim','_bitnet_get_num_layers','_bitnet_free_model','_bitnet_cleanup'] -s ALLOW_MEMORY_GROWTH=1 -s INITIAL_MEMORY=800MB -s MAXIMUM_MEMORY=4GB -s FORCE_FILESYSTEM=1 -s STACK_SIZE=16MB -s DISABLE_EXCEPTION_CATCHING=0 -s ASSERTIONS=0 -s USE_PTHREADS=0 -s PTHREAD_POOL_SIZE=0 --bind -s ERROR_ON_UNDEFINED_SYMBOLS=0 -s MALLOC=emmalloc -s NO_EXIT_RUNTIME=1"
